...
import vinux.service.core.exception.ExceptionCode;
	...
	public Object executeMethod(String... type) {
		try {
			...
			try {
				...
				VException ve = new VException(e, "请求[" + url + "]超时或错误,message : "
						+ e.getMessage());
				ve.setCode(ExceptionCode.RUN_TIME);//运行异常
				throw ve;
			}
			...
			if (status == HttpStatus.SC_OK) {
				...
				LoggerUtils.error(getClass(), "HTTP请求错误，请求地址为：[" + url + "],状态为：["
						+ status + "]");
				throw new VException("H" + status, "HTTP请求错误，请求地址为：[" + url
						+ "],状态为：[" + status + "]");
			}
			if (StringUtils.indexOf(JSONObject, type) > 0) {
				return net.sf.json.JSONObject.fromObject(result,getConfig());
			}
	
			if (StringUtils.indexOf(JSONArray, type) > 0) {
				return net.sf.json.JSONArray.fromObject(result,getConfig());
			}
			return result;
		}finally{
